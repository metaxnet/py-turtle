...
clip_y_origin=-1, graphics_exposures=-1,line_width=-1, line_style=-1, cap_style=-1,join_style=-1)
    ...
    def construct_list(self, parameter1, parameter2):
        parameters = [parameter1, parameter2]
        l = []
        for p in parameters:
            if p[0] == "\"":
    ...
    def _calculate_line_and_frame_instersections(self, sx, sy, ex, ey, H, W, a, b):
    ...
    def _calculate_visible_segment(self, sx, sy, ex, ey):
        ...
        points = self._calculate_line_and_frame_instersections(sx, sy, ex, ey, H, W, a, b)
        ...
        if self._is_visible((sx, sy)):
            ...
            distance = math.sqrt((sx - points[0][0]) ** 2 + (sy - points[0][1]) ** 2)
            closest = points[0]
            for i in range(len(points)):
                #print "POINT, CLOSEST (y):", points[i], closest
                if math.sqrt((sx - points[i][0]) ** 2 + (sy - points[i][1]) ** 2) < distance:
                    closest = points[i]
                    distance = math.sqrt((sx - points[i][0]) ** 2 + (sy - points[i][1]) ** 2)
            line_sx = closest[0]
            line_sy = closest[1]
        ...
        if self._is_visible((ex, ey)):
            ...
            distance = math.sqrt((ex - points[0][0]) ** 2 + (ey - points[0][1]) ** 2)
            closest = points[0]
            for i in range(len(points)):
                #print "POINT, CLOSEST (e):", points[i], closest
                if math.sqrt((ex - points[i][0]) ** 2 + (ey - points[i][1]) ** 2) < distance:
                    closest = points[i]
                    distance = math.sqrt((ex - points[i][0]) ** 2 + (ey - points[i][1]) ** 2)
            line_ex = closest[0]
            line_ey = closest[1]
    ...
    def _move_to_position(self, endx, endy):
        ...
        #print "Pen_position=", self.pen_position
        startx, starty = self.turtle_position
        w = self.screen_width + 0.0
        h = self.screen_height + 0.0
        #
        if self.draw_mode == MODE_WRAP and self.pen_position == PEN_UP:
            ...
            if self.pen_position == PEN_DOWN:
                self.area.window.draw_line(self.gc, startx, starty, endx, endy)
            ...
            if self.pen_position == PEN_DOWN:
                ...
                if segment:
                    ...
                    self.area.window.draw_line(self.gc, line_startx, line_starty, line_endx, line_endy)
        #
    ...
    def handle_turtle_query(self, query):
        x = self.turtle_position[0] - self.turtle_home_position[0]
        if x == int(x):
            x = int(x)
        y = self.turtle_position[1] - self.turtle_home_position[1]
        if y == int(y):
            y = int(y)
        if query == "xpos":
            return x
    ...
    def set_draw_mode(self, mode):
        self.draw_mode = mode
        if mode == WINDOW_MODE:
            x = self.turtle_position[0] % self.screen_width
            y = self.turtle_position[1] % self.screen_height
    ...
    def set_x(self, x):
        return self._move_to_position(x + self.turtle_home_position[0], self.turtle_position[1])

    def set_y(self, y):
        return self._move_to_position(self.turtle_position[0], y + self.turtle_home_position[1])
